...
$schema = file_text($root, 'database/lab5_products.sql');
...
expect_true(str_contains($auth_controller, 'AUTH_PASSWORD'), 'auth password configuration missing');
expect_true(!str_contains($auth_controller, 'admin123'), 'default password must not be hard-coded');
expect_true(str_contains($controller, 'LabDatabaseSetup::ensure'), 'products setup is not initialized');
expect_true(str_contains($setup, 'CREATE TABLE IF NOT EXISTS products'), 'automatic products setup missing');
