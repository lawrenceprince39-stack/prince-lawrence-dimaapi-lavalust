...
$schema = file_text($root, 'database/lab4_users.sql');
$setup = file_text($root, 'app/libraries/LabDatabaseSetup.php');
...
expect_true(str_contains($schema, 'CREATE DATABASE IF NOT EXISTS mydb'), 'mydb database schema missing');
expect_true(str_contains($schema, 'CREATE TABLE IF NOT EXISTS users'), 'users table schema missing');
expect_true(substr_count($schema, '@example.com') >= 5, 'at least five sample users are required');
expect_true(str_contains($setup, 'CREATE DATABASE IF NOT EXISTS mydb'), 'automatic mydb setup missing');
expect_true(str_contains($controller, 'LabDatabaseSetup::ensure'), 'users setup is not initialized');
