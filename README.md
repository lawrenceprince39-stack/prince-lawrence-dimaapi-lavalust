# Prince Lawrence Dimaapi - LavaLust Labs 1-5

This repository is a cumulative Web Systems and Technologies 2 project for Prince Lawrence Dimaapi. It contains the LavaLust setup and deployment foundation from Labs 1-2, the protected Student Information flow from Lab 3, the database-backed Users MVC page from Lab 4, and the authenticated Products CRUD application from Lab 5.

## Implemented laboratory routes

| Lab | Route | Result |
|---|---|---|
| 2 | `/` | Personalized LavaLust landing page |
| 3 | `/student` | Student Information page and session access grant |
| 3 | `/student/profile` | Student Profile protected by `StudentMiddleware` |
| 4 | `/users` | Dynamic users table populated through `UsersModel::all()` |
| 5 | `/login` | Environment-configured login |
| 5 | `/products` | Authenticated product list |
| 5 | `/products/create` | Authenticated create form |
| 5 | `/products/edit/{id}` | Authenticated edit form |
| 5 | `/products/delete/{id}` | Authenticated POST-only delete action |

See `LABS_1_TO_5.md` for local setup, database import, Render/Aiven configuration, tests, and the exact screenshot checklist. Do not commit `.env`; it contains private credentials.
